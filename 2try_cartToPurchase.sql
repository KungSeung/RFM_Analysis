...
reorders_user as (
  select 
      user_id,
      country,
      MAX(purchase_cnt) as max_purchase_cnt
  from pur_cnt
  where (event_type = 'cart') and (next_event_type = 'purchase')
  group by user_id, country
  having MAX(purchase_cnt) = 2  -- max_purchase_cnt가 1인 값만 선택
),
...
more1_orders_cnt as (
  select 
    ...
    SUM(case 
            when status IN ('Shipped', 'Processing', 'Complete') then 1 
            else 0 
        end) 
    OVER (partition by cast(user_id as int64) ORDER BY created_at) as order_cnt
  from looker-ecommerce-441405.Kaggle_Data_Processing.more1_orders
)

select 
    moc.*,
    ru.country
from more1_orders_cnt as moc
inner join reorders_user as ru
on ru.user_id = moc.user_id
order by moc.user_id;
-- select *
-- from more1_orders_cnt
